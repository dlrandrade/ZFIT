...
const WeeklyStats: React.FC<WeeklyStatsProps> = ({ primaryColor, progress, theme }) => {
  ...
  return (
    <div className="px-6 mb-12 relative z-10">
      <div
        className={`rounded-[40px] p-8 border shadow-xl transition-all duration-500 ${depthClass}`}
        style={{
      ...
      >
        <h2 className="text-[10px] font-black opacity-40 uppercase tracking-[0.3em] mb-8">Consistência Semanal</h2>
        <div className="flex items-center justify-between">
          <div className="flex items-baseline space-x-3">
            <span className="text-5xl font-black tracking-tighter">{progress}%</span>
            <div className="px-3 py-1 rounded-full flex items-center space-x-1.5 bg-black/10">
              <div className="w-0 h-0 border-l-[5px] border-l-transparent border-r-[5px] border-r-transparent border-b-[8px] border-b-black" style={{ borderBottomColor: isMint ? 'white' : 'black' }} />
          ...
          <div className="flex items-end space-x-1.5 h-12">
            {[6, 12, 18, 10, 14, 22, 16, 10].map((h, i) => (
              <div
                ...
                className="w-2 rounded-full bg-black/20 hover:bg-black transition-all cursor-pointer"
              />
            ))}
          </div>
        </div>
