...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="max-w-sm w-full space-y-8 relative z-10 animate-in fade-in zoom-in duration-700">
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/5">
          <button 
            ...
            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${!isSignUp ? 'bg-[#adf94e] text-black shadow-lg shadow-[#adf94e]/10' : 'text-white/40'}`}
          >
            Entrar
          </button>
          <button 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {error && (
            <div className="bg-red-500/10 border border-red-500/20 text-red-500 p-4 rounded-2xl text-[10px] font-black uppercase text-center animate-shake">
              {error}
            </div>
          )}
            ...
            {isSignUp && (
              <div className="relative group animate-in slide-in-from-top-2 duration-300">
                ...
                <input 
                  ...
                  className="w-full h-16 bg-white/5 rounded-[22px] border border-white/5 pl-16 pr-6 font-black uppercase text-xs tracking-widest focus:border-[#adf94e]/30 outline-none transition-all focus:bg-white/[0.08]"
                />
              </div>
            )}
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full h-16 bg-[#adf94e] rounded-[22px] flex items-center justify-center space-x-3 active:scale-95 transition-all disabled:opacity-50 shadow-[0_10px_30px_rgba(173,249,78,0.2)]"
          >
            {loading ? (
        ...
        <div className="text-center pt-4">
          <p className="text-[9px] font-black uppercase tracking-widest opacity-20 leading-relaxed">
