      ...
      <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-4xl mx-auto pb-12">
        ...
        <div className="px-6 mb-6 space-y-6">
          ...
          <div
            ...
            className={`rounded-[40px] p-8 border cursor-pointer group relative overflow-hidden transition-all hover:scale-[1.01] active:scale-[0.99] ${depthClass}`}
            style={{ backgroundColor: theme.card, borderColor: theme.border }}
          >
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center border" style={{ backgroundColor: theme.cardSecondary, borderColor: theme.border, color: theme.primary }}>
                  <BookOpen size={16} />
                </div>
            ...
            <div className="space-y-6">
              <div className="max-w-md">
                <h3 className="text-2xl font-black uppercase tracking-tighter leading-[1.1] mb-2" style={{ color: theme.text }}>
                  {latestArticle.title}
                </h3>
                <p className="text-[11px] font-medium leading-relaxed opacity-50 line-clamp-2" style={{ color: theme.text }}>
                  {latestArticle.excerpt}
                </p>
              </div>
              <div className="w-full h-56 rounded-[30px] overflow-hidden border relative group" style={{ borderColor: theme.border }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            />
            <div className={`rounded-[30px] p-6 border flex flex-col justify-between ${depthClass}`} style={{ backgroundColor: theme.card, color: theme.text, borderColor: theme.border }}>
              <div>
                <span className="opacity-40 text-[10px] uppercase font-black tracking-widest block mb-2">Esforço</span>
                <h3 className="text-2xl font-black tracking-tighter">{calories} kcal</h3>
              </div>
            </div>
          </div>
        </div>
                  ...
                  currentPage === 'feed' ? <Feed theme={theme} posts={socialFeed} onUpdatePosts={setSocialFeed} primaryColor={theme.primary} /> :
                    currentPage === 'stats' ? <StatsPage primaryColor={theme.primary} history={workoutHistory} /> :
                      currentPage === 'profile' ? <ProfilePage theme={theme} user={user} onUpdateUser={setUser} history={workoutHistory} onLogout={handleLogout} onOpenPricing={() => setCurrentPage('pricing')} /> :
                        currentPage === 'admin' ? <AdminDashboard theme={theme} onClose={() => setCurrentPage('home')} /> :
